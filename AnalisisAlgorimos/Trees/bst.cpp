...
nodo* sucesorInorder(nodo* T, nodo* root, int n)
{
	if (!T)
		return T;
	if (T->dato == n)
	{
		if (T->der)
			return minNodo(T->der);
	}
	if (n <= T->dato)
		T->izq = sucesorInorder(T->izq, root, n);
	else 
		T->der = sucesorInorder(T->der, root, n);
}
...
int main(void)
{
	...
	cout<<endl<<endl;
	preOrder(T3);
	nodo* inorderSuccessor = sucesorInorder(T, T, 13);
	cout<<endl<<endl<<inorderSuccessor->dato<<endl;
	return 0;
}